...
describe("resolvePackageFiles — registry-first dispatch", () => {
  it("with NO items for an auto-package slot, produces no files", () => {
    const preset = presetWithVercelAnalytics();
    const nodes = resolvePackageFiles(preset, analyticsPkg);
    // Post Eta-removal: no item, no output. The CLI is responsible for
    // surfacing the missing slot to the user (analogous to `unmet` in
  ...
  it("with a non-matching item, produces no files for that slot", () => {
    const preset = presetWithVercelAnalytics();
